  ...
  return (
    <Layout>
      <div className="mx-auto max-w-6xl px-4 py-8">
        <Link
          to="/listings"
          className="mb-6 inline-flex items-center gap-1 text-sm text-gray-500 hover:text-brand-500 transition"
        >
        ...
        <div className="grid gap-8 lg:grid-cols-5">
          ...
          <div className="lg:col-span-3">
            <div className="relative">
              {listing.screenshots.length > 0 ? (
                <div
                  className="group relative aspect-video cursor-pointer overflow-hidden rounded-xl bg-gray-100 dark:bg-navy-800"
                ...
                <div className="flex aspect-video items-center justify-center rounded-xl bg-gray-100 dark:bg-navy-800">
                  <span className="text-gray-400">Tidak ada screenshot</span>
                </div>
              )}
            </div>
            ...
            <div className="mt-8">
                  ...
                  <DetailSection title="Karakter 5-Star" icon={Star}>
                    <div className="flex flex-wrap gap-1.5">
                      {listing.five_stars.map((char, i) => (
                        <span key={i} className="rounded-full bg-brand-50 dark:bg-brand-900/30 px-2.5 py-1 text-xs font-medium text-brand-700 dark:text-brand-300">
                          {char}
                        </span>
                      ))}
                    </div>
                  ...
                  <DetailSection title="Senjata 5-Star" icon={Sword}>
                    <div className="flex flex-wrap gap-1.5">
                      {listing.weapons.map((w, i) => (
                        <span key={i} className="rounded-full bg-blue-50 dark:bg-blue-900/30 px-2.5 py-1 text-xs font-medium text-blue-700 dark:text-blue-300">
                          {w}
                        </span>
                      ))}
                    </div>
              ...
              {listing.artifacts && (
                <div className="mt-4 rounded-lg bg-gray-50 dark:bg-navy-800/50 p-4">
                  <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Artefak</p>
          ...
          <div className="lg:col-span-2">
            <div className="sticky top-24 space-y-6">
              <div className="rounded-xl border border-gray-200 dark:border-navy-700 bg-white dark:bg-navy-800 p-6 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                  <Badge variant={SERVER_COLORS[listing.server] || 'info'} size="sm">
                    {SERVER_LABELS[listing.server]}
                  </Badge>
                  <Badge variant="neutral" size="sm">AR {listing.ar}</Badge>
                  {listing.status === 'sold' && <Badge variant="danger" size="sm">Terjual</Badge>}
                </div>

                <h1 className="text-xl font-bold text-navy-800 dark:text-gray-100 mb-4">
                  {listing.title}
                </h1>

                <p className="text-3xl font-extrabold text-brand-500 dark:text-brand-400 mb-6">
                  {formatPrice(listing.price)}
                </p>

                {listing.seller && (
                  <div className="flex items-center gap-3 rounded-lg bg-gray-50 dark:bg-navy-900 p-3 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-green-600 dark:text-green-400">Penjual Terverifikasi</p>
                    </div>
                  </div>
                )}
        ...
        <div className="space-y-4">
          ...
          {purchaseModal === 'auto' && (
            <>
              ...
              {/* TODO: Integrate Midtrans payment gateway here */}
          ...
          <div className="rounded-lg bg-gray-50 dark:bg-navy-900 p-4">
            ...
            <div className="flex justify-between text-sm mt-2">
              ...
              <span className="font-bold text-brand-500">{formatPrice(listing.price)}</span>
            </div>
          </div>
...
function DetailSection({ title, icon: Icon, children }: { title: string; icon: React.ElementType; children: React.ReactNode }) {
  return (
    <div className="rounded-lg bg-gray-50 dark:bg-navy-800/50 p-4">
...
function StatBox({ icon: Icon, label, value, color }: { icon: React.ElementType; label: string; value: string; color?: string }) {
  return (
    <div className="rounded-lg bg-gray-50 dark:bg-navy-800/50 p-3 text-center">
      <Icon className={`mx-auto h-5 w-5 mb-1 ${color === 'green' ? 'text-green-500' : 'text-gray-400'}`} />
...
function InfoItem({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: string }) {
  return (
    <div className="flex items-center gap-2 rounded-lg bg-gray-50 dark:bg-navy-800/50 p-3">
      <Icon className="h-4 w-4 text-gray-400 flex-shrink-0" />
      <div>
        <p className="text-[10px] text-gray-400">{label}</p>
        <p className="text-sm font-medium text-navy-800 dark:text-gray-100">{value}</p>
      </div>
    </div>
  );
}
